...
export const apiRequestTool = new DynamicTool({
  name: "api_request",
  description: "실제 API 요청을 수행합니다. API 호출이 필요할 때는 반드시 이 도구를 사용하세요. 코드 작성이 아닌 실제 API 호출만 수행합니다. 입력 형식: JSON 문자열 {\"url\": \"요청URL\", \"method\": \"GET|POST|PUT|DELETE\", \"headers\": {}, \"data\": {}}",
  func: async (inputStr) => {
    try {
      // 입력 문자열을 JSON으로 파싱
      const input = JSON.parse(inputStr);
      const { url, method = 'GET', headers = {}, data = null, params = null } = input;
